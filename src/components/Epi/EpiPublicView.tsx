...
export const EpiPublicView: React.FC<EpiPublicViewProps> = ({ token, onAdminBack }) => {
  ...
  return (
    <div className="min-h-screen bg-[#111111] text-slate-100 flex flex-col font-sans selection:bg-[#C48229] selection:text-white print:bg-white print:text-black">
      {/* Header */}
        ...
        <div className="flex items-center gap-3">
          {dados && (
            <>
              ...
              >
                {baixando ? <Loader2 className="w-3.5 h-3.5 animate-spin text-[#C48229]" /> : <Download className="w-3.5 h-3.5 text-[#C48229]" />}
              ...
              >
                <Printer className="w-3.5 h-3.5 text-[#C48229]" />
                <span>Imprimir</span>
              </button>
            </>
          )}
