...
import {
  FichaCompartilhada,
  gerarLinkFichaCompartilhada,
  listarLinksFichaCompartilhada,
  revogarLinkFichaCompartilhada,
} from '../../utils/dpApi';
import { formatDate } from '../../utils/formatters';

interface CompartilharFichaModalProps {
  isOpen: boolean;
  onClose: () => void;
...
function montarUrlFicha(token: string): string {
  const base = `${window.location.origin}${window.location.pathname}`;
  return `${base}?form=ficha&token=${token}`;
}

/** Gera/gerencia links de compartilhamento da ficha cadastral (pra terceiro externo — contador,
 *  fiscalização, RH de outra empresa — sem precisar de login). Cada link é um "instantâneo" dos
 *  dados no momento da geração, expira em 7 dias e pode ser revogado antes disso. */
export const CompartilharFichaModal: React.FC<CompartilharFichaModalProps> = ({
  isOpen,
  onClose,
  colaborador,
  criadoPor,
}) => {
  const [links, setLinks] = useState<FichaCompartilhada[]>([]);
  ...
  const carregarLinks = async () => {
    ...
    setErro(null);
    try {
      const lista = await listarLinksFichaCompartilhada(colaborador.id);
      setLinks(lista);
    } catch (e) {
      console.error('Erro ao listar links da ficha compartilhada:', e);
  ...
  const handleGerarLink = async () => {
    ...
    setErro(null);
    try {
      await gerarLinkFichaCompartilhada(colaborador, criadoPor, 7);
      await carregarLinks();
    } catch (e) {
      console.error('Erro ao gerar link da ficha compartilhada:', e);
  ...
  const handleCopiar = async (token: string) => {
    try {
      await navigator.clipboard.writeText(montarUrlFicha(token));
  ...
  const handleRevogar = async (token: string) => {
    ...
    try {
      await revogarLinkFichaCompartilhada(token);
      await carregarLinks();
    } catch (e) {
        ...
        <div className="p-5 space-y-4 overflow-y-auto flex-1">
          <div className="bg-amber-50 border border-amber-200 rounded-xl p-3 flex gap-2.5">
            ...
            <p className="text-[11px] text-amber-800 leading-relaxed">
              O link mostra uma <strong>foto dos dados de agora</strong> (não muda se você editar o cadastro depois),
              expira em <strong>7 dias</strong> e pode ser revogado a qualquer momento. Use só pra compartilhar com
              quem realmente precisa ver esses dados (contador, fiscalização etc.).
            </p>
          </div>
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex items-center gap-2">
                      <input
                        readOnly
                        value={montarUrlFicha(link.token)}
