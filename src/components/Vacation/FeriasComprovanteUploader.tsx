...
interface FeriasComprovanteUploaderProps {
...
/** Anexo do recibo/aviso de férias assinado pelo colaborador — mesmo padrão de upload
 * (FileReader → data URL em localStorage) já usado no ASO digitalizado (AsoImageUploader). */
export const FeriasComprovanteUploader: React.FC<FeriasComprovanteUploaderProps> = ({
  comprovanteUrl,
  nomeArquivo,
  onUpload,
  onRemove,
  ...
  // Registro antigo: existe nome de arquivo (ex.: vindo de um arquivamento físico), mas nenhum
  // arquivo digital foi de fato anexado ainda.
  const isRegistroSemArquivoDigital = !comprovanteUrl && !!nomeArquivo;
  ...
  return (
    <div className="bg-slate-50 p-3.5 rounded-xl border border-slate-200">
      <div className="flex items-center justify-between mb-2">
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-xs font-bold text-slate-800">
            Comprovante de Férias Assinado pelo Colaborador
          </span>
        </div>
        {comprovanteUrl && (
      ...
      {comprovanteUrl ? (
        <div className="flex flex-col sm:flex-row items-center gap-3 bg-white p-3 rounded-lg border border-slate-200">
          ...
          >
            {isPdf ? (
              ...
              <img
                src={comprovanteUrl}
                alt="Comprovante de férias assinado"
          ...
          <div className="flex-1 min-w-0 text-left">
            <div className="font-semibold text-xs text-slate-800 truncate">
              {nomeArquivo || 'Comprovante_Ferias_Assinado'}
            </div>
            <p className="text-[11px] text-slate-500 mt-0.5">
              Recibo/aviso de férias com a ciência e assinatura do colaborador, para fins de comprovação CLT.
            </p>
              ...
              {!readOnly && (
                <>
                  ...
                  <button
                    ...
                    title="Remover comprovante anexado"
                  >
        ...
        <>
          {isRegistroSemArquivoDigital && (
            <div className="flex items-center gap-2.5 bg-white p-2.5 rounded-lg border border-slate-200 mb-2">
              <Archive className="w-4 h-4 text-slate-400 shrink-0" />
              <div className="flex-1 min-w-0 text-left">
                <div className="font-semibold text-[11px] text-slate-700 truncate">{nomeArquivo}</div>
                <p className="text-[10px] text-slate-400">
                  Registro de arquivamento anterior, sem cópia digital anexada.
                </p>
              </div>
              {!readOnly && (
                <button
                  type="button"
                  onClick={onRemove}
                  className="p-1 text-rose-500 hover:bg-rose-50 rounded-md transition-colors shrink-0"
          ...
          >
            ...
            <span className="text-xs font-bold text-slate-800 block">
              {isRegistroSemArquivoDigital
                ? 'Clique para anexar a versão digital do comprovante'
                : 'Clique para anexar a foto ou PDF do comprovante assinado'}
            </span>
      ...
      <ImageViewerModal
        ...
        title="Comprovante de Férias Assinado"
        subtitle="Comprovação de ciência do colaborador — Departamento Pessoal (CLT)"
